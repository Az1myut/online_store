...
class SingleProductPageDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['from_view'] = parser.render(
            '[b]Здравствуйте, люди [u]дорогие![/u][/b]')
        return context
...
def products_bulk_edit(request):
    ProductFormSet = modelformset_factory(
        SingleProduct, form=ProductForm, fields=(
            'name', 'description', 'in_store', 'initial_price'),
    ...
    if request.method == 'POST':
        ...
        formset = ProductFormSet(
            queryset=SingleProduct.objects.prefetch_related('images'))
        '''
...
class ProductImageBulkEditListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['product_image_formset'] = self.ProductImgaeFormset(
